...
var dataDA= JSON.parse(localStorage.getItem("dato"));
var dataTA= JSON.parse(localStorage.getItem("gasto"));
var dataCU= JSON.parse(localStorage.getItem("gasto"));
var conCU= JSON.parse(localStorage.getItem("cuentas"));
var conTA= JSON.parse(localStorage.getItem("tarjeta"));
var conteo = JSON.parse(localStorage.getItem("contga"));
...
if (dataDA==null) {
	...
	if (selec=="Efectivo") {
		...
		var tot = ((parseInt(pisto[idxd]))-(parseInt(monto)));
		pisto[idxd]=tot;
	...
	else if(selec=="Tarjeta de credito"){
		var cuenta=document.getElementById("tarjeta").value;

	var prov0=tarjetasG[idxd][cuenta][0];
	var prov1=tarjetasG[idxd][cuenta][1];

	var prov2=tarjetasG[idxd][cuenta][2];
	var prov3=tarjetasG[idxd][cuenta][3];
	var prov4=tarjetasG[idxd][cuenta][4];
	var provT=((parseInt(prov0))+(parseInt(monto)));
		tarjetasG[idxd][cuenta]=[provT,prov1,prov2,prov3,prov4];

		cuentaa=tarjetasG[idxd][cuenta][1];
		localStorage.setItem("tarjeta", JSON.stringify(tarjetasG));
  ...
  localStorage.setItem("gasto", JSON.stringify(cuentasG));
  localStorage.setItem("contga", JSON.stringify(conteo));
...
function ver(){
	...
	var selec = document.getElementById("medio").value;
	if (selec=="Efectivo") {
		document.getElementById("metodo2").style="opacity: 0;";
		document.getElementById("metodo").style="opacity: 0;";
	}
	else if(selec=="Cuenta bancaria"){
		document.getElementById("metodo2").style=" opacity: 1;";
		document.getElementById("metodo").style="opacity: 0;";
	}
	else if(selec=="Tarjeta de credito"){
		document.getElementById("metodo2").style=" opacity: 0;";
